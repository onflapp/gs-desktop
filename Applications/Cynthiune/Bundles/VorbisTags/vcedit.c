// slightly modified by Yen-Ju Chen <yjchenx@hotmail.com>
 ...
 *
 * last modified: $Id: vcedit.c,v 1.1.1.1 2012/03/25 07:48:23 buzzdee Exp $
 */
		...
		vcedit_read_func read_func, vcedit_write_func write_func)
{
	...
	{
		if(bytes<CHUNKSIZE)
			printf("Input truncated or empty.");
		else
			printf("Input is not an Ogg bitstream.");
		goto err;
	}
	...
	{
		printf("Error reading first page of Ogg bitstream.");
		goto err;
	}
	...
	{
		printf("OGG bitstream does not contain vorbis data.");
		goto err;
	}
	...
	while(i<2) {
		while(i<2) {
			...
			else if(result == 1)
			{
				...
				while(i<2)
				{
					...
					if(result == -1)
					{
						printf("Corrupt secondary header.");
						goto err;
					}
		...
		if(bytes == 0 && i < 2)
		{
			printf("EOF before end of vorbis headers.");
			goto err;
		}
		ogg_sync_wrote(state->oy, bytes);
	}
	...
	while(!state->eosin) /* We reached eos, not eof */
	{
		...
		while(1)
		{
			...
			if(result==0)
                break;
			if(result<0)
				printf("Corrupt or missing data, continuing...");
			else
			{
...
cleanup:
	...
	if(!state->eosin)
	{
		printf("Error writing stream to output. Output stream may be corrupted or truncated.");
		return -1;
	}
