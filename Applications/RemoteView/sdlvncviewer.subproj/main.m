/**
 * taken from the SDLvncviewer.c example
 */
...
static rfbBool handleSDLEvent(rfbClient *cl, SDL_Event *e)
{
	...
	case SDL_WINDOWEVENT:
	    ...
	    case SDL_WINDOWEVENT_RESIZED:
		      //might require newer version of vncclient lib
	    ...
	    case SDL_WINDOWEVENT_FOCUS_GAINED:
			...
			if(text) {
			    NSLog(@"sending clipboard text '%s'\n", text);
		...
		if (rightAltKeyDown) {
			...
			NSLog(@"released right Alt key\n");
		}
		if (leftAltKeyDown) {
			SendKeyEvent(cl, XK_Alt_L, FALSE);
			leftAltKeyDown = FALSE;
			NSLog(@"released left Alt key\n");
		}
		break;
	    }
	    break;
...
{
        NSLog(@"received latin1 clipboard text '%s'\n", text);
...
{
        NSLog(@"received utf8 clipboard text '%s'\n", buf);
